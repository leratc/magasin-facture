...
export class CommandeProduitComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.commandeProduitService
      ...
      .subscribe(
        (res: HttpResponse<ICommandeProduit[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  ngOnInit(): void {
    ...
    this.registerChangeInCommandeProduits();
  }
  ...
  registerChangeInCommandeProduits(): void {
    this.eventSubscriber = this.eventManager.subscribe('commandeProduitListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ICommandeProduit[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/commande-produit'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
